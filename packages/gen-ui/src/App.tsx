...
// Transposition options - just the key name to pass to the backend
const TRANSPOSE_OPTIONS = [
  { label: "C (Concert Pitch)", key: "C" },
  { label: "Bb Instruments", key: "Bb" },
  { label: "Eb Instruments", key: "Eb" },
  { label: "F Instruments", key: "F" },
] as const;
...
function App() {
  ...
  const compileAndRender = useCallback(async (source: string, viewedKey: string) => {
    if (!source.trim()) return;

    setIsCompiling(true);
    try {
      // Use backend transposition
      const result = await invoke<CompileResult>("compile_gen_transposed", { source, viewedKey });
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      compileAndRender(genSource, TRANSPOSE_OPTIONS[transposeIndex].key);
    }, 150);
